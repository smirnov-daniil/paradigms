...
public class IntegerEvaluator implements Evaluator<Integer> {
    ...
    public Integer mul(Integer a, Integer b) {
        if (CheckedMultiply.checkOverflow(a, b)) {
            throw overflow(a, b, "*");
        }
        return a * b;
    }
    ...
    public Integer div(Integer a, Integer b) {
        if (CheckedDivide.checkOverflow(a, b)) {
            throw overflow(a, b, "/");
        }
        if (b == 0) {
            throw divByZero(a);
    ...
    public Integer neg(Integer a) {
        if (CheckedNegate.checkOverflow(a)) {
            throw overflow(a);
        }
        return -a;
    }
    ...
    private expression.exceptions.ExpressionOverflowException overflow(Integer a, Integer b, String op) {
        return new expression.exceptions.ExpressionOverflowException(String.format("%d %s %d", a, op, b));
    }

    private expression.exceptions.ExpressionOverflowException overflow(Integer a) {
        return new expression.exceptions.ExpressionOverflowException("-" + a);
    }
}
